...
import { categories, learningPath, nodes, relations, robotTypes, type Category, type Node, type RobotType, type Source } from './data/industry';
...
function SourceStatus({ source }: { source: Source }) {
  ...
  return <div className="source-status">
    <span className={`verification ${verified ? 'verified' : 'pending'}`}>{verified ? '已核实' : '待核实'}</span>
    <small>{verified ? `核实日期：${source.accessed ?? source.checkedAt}` : `最近尝试：${source.checkedAt}`}</small>
    {source.published && <small>发布日期：{source.published}</small>}
    <p>{verified ? '证据摘要' : '待核实范围'}：{source.evidence}</p>
    {source.note && <p className="verification-note">{verified ? '适用边界：' : '待核实原因：'}{source.note}</p>}
  </div>;
}
...
function Detail({ node, close, select }: { node: Node; close:()=>void; select:(id:string)=>void }) {
  ...
  useEffect(() => {
    const trigger = document.activeElement as HTMLElement | SVGElement | null;
    const background = document.getElementById('site-content');
    const previousInert = background?.inert ?? false;
    const previousOverflow = document.body.style.overflow;
    if (background) background.inert = true;
    document.body.style.overflow = 'hidden';
    panel.current?.querySelector<HTMLButtonElement>('.close')?.focus();
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === 'Tab') {
        const controls = panel.current?.querySelectorAll<HTMLElement>('button:not([disabled]), a[href], input, select, [tabindex="0"]');
  ...
  return <aside className="detail" role="dialog" aria-modal="true" aria-label={`${node.name}详情`} ref={panel}>
    ...
    <section><h3>企业与具体产品</h3>{node.companies.map(c=><article className="company" key={`${c.name}-${c.product}`} aria-label={`${c.name}产品资料`}><div className="company-heading"><b>{c.name}</b><span>{c.region}</span></div><p>{c.product}</p><SourceStatus source={c.source}/><a href={c.source.url} target="_blank" rel="noreferrer">产品原始资料 <ExternalLink/></a></article>)}</section>
    <section><h3>资料来源</h3><p className="source-note"><Info/> 按资料逐条标注核实状态与证据范围；产品资料不代表已确认企业间供货。</p>{node.sources.map(s=><article className="source" key={s.url} aria-label={s.title}><a href={s.url} target="_blank" rel="noreferrer"><span><b>{s.title}</b><small>{s.publisher}</small></span><ExternalLink/></a><SourceStatus source={s}/></article>)}</section>
  </aside>
}
...
function Map({ visible, selected, select }: { visible:Node[]; selected?:string; select:(id:string)=>void }) {
  ...
  useEffect(() => {
    ...
    if (!element) return;
    const resize=()=>setBaseWidth(Math.max(920, element.clientWidth));
    resize();
  ...
  const reset=()=>{setZoom(100);if(viewport.current){viewport.current.scrollLeft=0;viewport.current.scrollTop=0}};
  const visibleIds=new Set(visible.map(n=>n.id));
  const related=new Set(relations.filter(r=>r.from===selected||r.to===selected).flatMap(r=>[r.from,r.to]));
  return <div className="map-wrap">
    <div className="map-tools"><button onClick={()=>setZoom(z=>Math.min(135,z+15))} disabled={zoom===135} aria-label="放大"><Plus/></button><button onClick={()=>setZoom(z=>Math.max(70,z-15))} disabled={zoom===70} aria-label="缩小"><Minus/></button><button onClick={reset} aria-label="复位"><RotateCcw/></button><output aria-label="地图缩放">{zoom}%</output></div>
    <div className="map-scroll" ref={viewport}><svg className="map" viewBox="0 0 1180 680" style={{width:`${baseWidth*zoom/100}px`}} role="group" aria-label="机器人产业链地图">
      ...
      {categories.map((c,i)=><g key={c}><rect x={i*285+18} y="18" width="280" height="644" rx="20" fill={colors[c]} opacity=".045"/><text x={i*285+38} y="53" className="column-title" fill={colors[c]}>{String(i+1).padStart(2,'0')} · {c}</text></g>)}
      ...
      {visible.map(n=>{const dim=selected && n.id!==selected&&!related.has(n.id);return <g role="button" aria-label={n.name} tabIndex={0} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();select(n.id)}}} onClick={()=>select(n.id)} className="node" key={n.id} opacity={dim?.24:1} transform={`translate(${n.x},${n.y})`}><rect width="190" height="68" rx="13" fill="white" stroke={selected===n.id?colors[n.category]:'#dde2e9'} strokeWidth={selected===n.id?3:1.3}/><circle cx="20" cy="21" r="5" fill={colors[n.category]}/><text x="34" y="26" className="node-name">{n.name}</text><text x="20" y="49" className="node-sub">{n.subtitle}</text></g>})}
...
function App(){
 ...
 const sources=Array.from(new globalThis.Map(nodes.flatMap(n=>[...n.sources,...n.companies.map(c=>c.source)]).map(s=>[s.url,s])).values());
 const verifiedCount=sources.filter(s=>s.verification==='verified').length;
 return <div className="app"><div id="site-content"><header><a className="brand" href="#top"><span>ROBOT</span><b>知链</b></a><nav id="main-nav" className={mobileNav?'open':''}><a href="#map" onClick={()=>setMobileNav(false)}>产业图谱</a><a href="#path" onClick={()=>setMobileNav(false)}>学习路径</a><a href="#method" onClick={()=>setMobileNav(false)}>阅读说明</a></nav><button className="menu" onClick={()=>setMobileNav(!mobileNav)} aria-label="菜单" aria-controls="main-nav" aria-expanded={mobileNav}><Menu/></button></header>
 ...
 <section id="path" className="path-section"><div className="section-head light"><div><p className="kicker">BEGINNER'S PATH</p><h2>一份订单，如何带动机器人？</h2><p>从仓储搬运出发，用 6 步串起应用、整机、软件和零部件。</p></div><BookOpen/></div><div className="path-content"><div className="path-steps">{learningPath.map((s,i)=><button className={i===pathStep?'active':''} aria-current={i===pathStep?'step':undefined} onClick={()=>setPathStep(i)} key={s.node}><span>{i<pathStep?<Check/>:i+1}</span><p><b>{s.title}</b><small>{nodes.find(n=>n.id===s.node)?.name}</small></p></button>)}</div><article className="lesson" aria-live="polite"><span>第 {pathStep+1} / {learningPath.length} 步</span><h3>{learningPath[pathStep].title}</h3><p>{learningPath[pathStep].text}</p><button onClick={()=>choose(learningPath[pathStep].node)}>查看「{nodes.find(n=>n.id===learningPath[pathStep].node)?.name}」详情 <ArrowRight/></button><div><button disabled={pathStep===0} onClick={()=>setPathStep(x=>x-1)}>上一步</button><button disabled={pathStep===learningPath.length-1} onClick={()=>setPathStep(x=>x+1)}>下一步</button></div></article></div></section>
 <section id="method" className="method"><p className="kicker">HOW TO READ</p><h2>先看关系，再看公司</h2><div><article><Link2/><h3>技术依赖 ≠ 供货关系</h3><p>图中实线解释部件、数据和系统为何相连。只有企业原始资料明确披露时，才会标记“公开供货”。首版没有此类连线。</p></article><article><Info/><h3>来源透明</h3><p>共 {sources.length} 项独立来源：{verifiedCount} 项已核实，{sources.length-verifiedCount} 项待核实。每项资料标明证据范围及核实日期或最近尝试日期；待核实资料同时说明原因，不能当作已确认事实。</p></article><article><BookOpen/><h3>结构因类型而异</h3><p>机械臂强调关节与末端工具，移动机器人强调底盘和导航，人形机器人还要解决双足平衡；筛选器可分别查看。</p></article></div></section></main>
 <footer><div className="brand"><span>ROBOT</span><b>知链</b></div><p>面向初学者的机器人产业链学习地图 · 内容不构成采购或投资建议</p><a href="#top">回到顶部 ↑</a></footer></div>
 {selectedNode&&<div className="overlay" onMouseDown={e=>e.target===e.currentTarget&&closeDetail()}><Detail node={selectedNode} close={closeDetail} select={choose}/></div>}</div>
}
export default App;
